...
class NavTreeNodeImpl extends React.Component<IProps, IState> {
  ...
  public render() {
    ...
    let children: string[] = [];
    if (this.state.expanded) children = node.children || [];
    else if (node.kind === "DynamicCollection") {
      children = (node.children || []).filter(
        (id) => this.props.highlights.indexOf(id) !== -1
      );
    }
    ...
    const doclinkEl = (
      <Link to={`/doc/${this.props.id}`}>
        <span className={css.navtreenode_doclink}>{node.title}</span>
      </Link>
    );
    ...
    return (
      <div className={css.navtreenode}>
        <div className={titleClasses.join(" ")}>
          {node.children && node.children.length ? buttonEl : null}
          {doclinkEl}
        </div>
